...
import com.fork.app.domain.dto.request.CartOptionDto;
...
//    }
    ...
    public void addCart(CartRequestDto cartRequestDto, Long userId) {
        ...
        if (cart != null && !cart.getRestaurant().getRestaurantId().equals(restaurant.getRestaurantId())) {
            cartItemRepository.deleteAll(cart.getCartItems());
            cartRepository.delete(cart);
            cart = null;
        }
        ...
        // 항상 새로운 CartItem 추가 (옵션이 다르면 기존 것과 구별해야 하므로)
        CartItem cartItem = CartItem.builder()
                .cart(cart)
                .menu(menu)
                .quantity(cartRequestDto.getQuantity())
        ...
        // 옵션 저장
        List<CartOptionDto> selectedOptions = cartRequestDto.getSelectedOptions();
        if (selectedOptions != null && !selectedOptions.isEmpty()) {
            for (CartOptionDto optionDto : selectedOptions) {
                CartItemOption option = CartItemOption.builder()
                        .groupName(optionDto.getGroupName())
                        .optionName(optionDto.getOptionName())
                        .optionPrice(optionDto.getOptionPrice())
                        .cartItem(cartItem)
                        .build();
                // 저장 생략 가능 (cascade = PERSIST 설정되어 있다면)
                cartItem.getSelectedOptions().add(option);  // 옵션 리스트에 추가
            }
        }
    }
...
//                .collect(Collectors.toList());
        ...
        List<CartItemResponseDto> cartItemResponseList = cart.getCartItems().stream()
                .map(cartItem -> {
                    int optionExtraPrice = cartItem.getSelectedOptions().stream()
                    ...
                    return CartItemResponseDto.builder()
                            ...
                            .price(cartItem.getMenu().getPrice())
                            .totalPrice(itemTotalPrice)
                            .options(cartItem.getSelectedOptions().stream().map(opt -> {
                                Map<String, Object> map = new HashMap<>();
                                map.put("groupName", opt.getGroupName());
                                map.put("optionName", opt.getOptionName());
                                map.put("optionPrice", opt.getOptionPrice());  // int는 Object로 자동 업캐스팅됨
                                return map;
                            }).collect(Collectors.toList()))
                            .build();
                })
